...
def test_backtest_rejects_missing_context():
    start = datetime(2024, 1, 1)
    points = [TimeSeriesPoint(timestamp=start + timedelta(days=i), value=float(i)) for i in range(20)]
    dataset = TimeSeriesDataset(points=points, frequency="D")
    with pytest.raises(ValueError, match="context_events"):
        WalkForwardBacktester().run(
            dataset,
            LastValueModel(),
            WalkForwardConfig(horizon=2, step=1, min_train_size=5),
            recorder=NullRecorder(),
        )
